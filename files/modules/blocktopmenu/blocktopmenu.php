...
class Blocktopmenu extends Module
{
	...
	private function generateCategoriesMenu($categories, $is_children = 0)
	{
		...
		foreach ($categories as $key => $category)
		{
			...
			if($category['name']=='Product Collection')
                        $html .= '<a href="'.$link.'" title="'.$category['name'].'">'.$this->l('Browse Collections').'</a>';
                        else
                            if($category['name']=='Pfizer')
                                $html .= '<a href="'.$link.'" title="'.$category['name'].'">'.$this->l('Browse Categories').'</a>';
                            else
                                $html .= '<a href="'.$link.'" title="'.$category['name'].'">'.$category['name'].'</a>';

			if (isset($category['children']) && !empty($category['children']) && $category['level_depth']<3)
			{
				$html .= '<ul>';
