...
class AdminCustomerPreferencesControllerCore extends AdminController
{
	public function __construct()
	{
		...
		parent::__construct();
                ...
                $module = Module::getInstanceByName('activationbymail');
			if (Validate::isLoadedObject($module))
			{
				...
				{
					if (Tools::getValue('PS_VERIFY_EMAIL'))
						$module->enable();
					else
						$module->disable();
				}
		...
		$this->fields_options = array(
			'general' => array(
				...
				'fields' =>	array(
					...
					'PS_ALLOW_REGISTRATION' => array(
						'title' => $this->l('Allow New User Registration'),
						'hint' => $this->l('If you chose No, new user registration will be disabled'),
						'validation' => 'isBool',
						'cast' => 'intval',
						'type' => 'bool'
					),
					'PS_VERIFY_EMAIL' => array(
						'title' => $this->l('Email Verification'),
						'hint' => $this->l('If you chose Yes, email verification required before the account become active'),
						'validation' => 'isBool',
						'cast' => 'intval',
						'type' => 'bool'
					),
