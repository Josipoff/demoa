...
class ProductCore extends ObjectModel
{
	...
	public static $definition = array(
		...
		'fields' => array(
			...
			'item_number' => 					array('type' => self::TYPE_STRING, 'validate' => 'isReference', 'size' => 32),
			...
			'web_service' => 				array('type' => self::TYPE_BOOL, 'validate' => 'isBool'),
			'profit_margin' => 				array('type' => self::TYPE_FLOAT, 'validate' => 'isFloat'),
			'property_0' => 				array('type' => self::TYPE_BOOL, 'validate' => 'isBool'),
			'property_1' => 				array('type' => self::TYPE_BOOL, 'validate' => 'isBool'),
			'property_2' => 				array('type' => self::TYPE_BOOL, 'validate' => 'isBool'),
			'property_3' => 				array('type' => self::TYPE_BOOL, 'validate' => 'isBool'),
			'property_4' => 				array('type' => self::TYPE_BOOL, 'validate' => 'isBool'),
			'property_5' => 				array('type' => self::TYPE_BOOL, 'validate' => 'isBool'),
			'property_6' => 				array('type' => self::TYPE_BOOL, 'validate' => 'isBool'),
			'property_7' => 				array('type' => self::TYPE_BOOL, 'validate' => 'isBool'),
			'property_8' => 				array('type' => self::TYPE_BOOL, 'validate' => 'isBool'),
			'property_9' => 				array('type' => self::TYPE_BOOL, 'validate' => 'isBool'),
			'property_10' => 				array('type' => self::TYPE_BOOL, 'validate' => 'isBool'),
			'property_11' => 				array('type' => self::TYPE_BOOL, 'validate' => 'isBool'),
			...
			'youtube_link' => 				array('type' => self::TYPE_HTML, 'lang' => true, 'validate' => 'isCleanHtml'),
			'decoration_details' => 				array('type' => self::TYPE_HTML, 'lang' => true, 'validate' => 'isCleanHtml'),
			'quick_quote' => 				array('type' => self::TYPE_HTML, 'lang' => true, 'validate' => 'isCleanHtml'),
		...
		$id_customer = 0, $use_customer_price = true, $id_cart = 0, $real_quantity = 0)
	{
		...
		if ($price < 0)
			$price = 0;
                ...
                if(!$specific_price){
                  $getProductMargin = $this->getProfitMargin((int)$id_product);
                 $getProductMargin = floatval($getProductMargin[0]['profit_margin']);
                    ...
                    if(isset($margin) && $price_type == 1){
                        $price = $price * (1 + floatval($margin)/100.00);
                    }	
                }                
	...
	*/
	...
	public static function addCustomField($product_id,$type,$required,$name)
	{
		// Label insertion
		if (!Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'customization_field` (`id_product`, `type`, `required`)
			VALUES ('.(int)$product_id.', '.(int)$type.', '.(int)$required.')') ||
			!$id_customization_field = (int)Db::getInstance()->Insert_ID())
			return false;
		...
		// Multilingual label name creation
                $language_id=1;
		$values = '';
		$values .= '('.(int)$id_customization_field.', '.$language_id.', "'.$name .'"), ';
        ...
        public static function getProductIdByReference($reference)
	{
        ...
        public static function addAttachments($id_product,$id_attachment)
	{
		// Get all ids attachments of the old product
		$data = array();
        ...
        public static function getAllProductsIdAddedByWebservice()
	{
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT `id_product`
			FROM `'._DB_PREFIX_.'product` WHERE web_service = 1');
	}
        public static function getProfitMargin($id_product)
	{
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('SELECT '
                        . '`profit_margin` from `'._DB_PREFIX_.'product` WHERE id_product = "'.(int)$id_product.'"');
	}
        
        
        public static function deleteAllCustomField($product_id)
	{
		// Label insertion
		if (!Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'customization_field` WHERE `id_product` = "'.(int)$product_id.'"'))
			return false;
		return true;
	}
