...
$(document).ready(function()
{
	...
	openCloseFilter();
     ...
     $(document).on("keypress", ".price_input",function (e) {
    ...
    $(document).on("keyup", ".price_input",function(){        
        $('#layered_price_slider').slider('values',[$('#price_min').val(),$('#price_max').val()]);
           reloadContent(true);
    });
       $(".datepicker").datepicker({ minDate: 0,
       dateFormat: 'yy-mm-dd'});
   
                if($('#layered_quantity_3').is(':checked'))
                    $('#layered_quantity_available_date').removeAttr('disabled');
                else{
                    $('#layered_quantity_available_date').attr("disabled", "disabled");
                    $('#layered_quantity_available_date').val("");
                }
	...
	$(document).on('click', '#layered_form .select, #layered_form input[type=checkbox], #layered_form input[type=radio]', function(e) {
            if($(this).attr('id')=='layered_quantity_1' || $(this).attr('id')=='layered_quantity_3'){
                
                if($('#layered_quantity_range_min').val()=="")
                {
                    $('#layered_quantity_range_min').val("0");
                    reloadContent(true);
                }
	...
	// Changing content of an input text
        ...
        $(document).on('change', '#layered_quantity_available_date', function(e){
	...
	$(sliderList).each(function(i, slider){
		...
		switch (slider['format'])
		{
			...
			case 5:
                ...
                $("#price_min").val($('#layered_'+slider['type']+'_slider').slider('values', 0));
                $("#price_max").val($('#layered_'+slider['type']+'_slider').slider('values', 1));
...
function reloadContent(params_plus)
{
	...
	{
		if (params_plus)
			data += '&id_manufacturer=' + $('#manufacturer_id').val()+'&';
	}
        if ($('#new_products').length)
	{
		if (params_plus)
			data += '&new_products=true&';
	}
        if ($('#price_drop').length)
	{
		if (params_plus)
			data += '&price_drop=true&';
	}
