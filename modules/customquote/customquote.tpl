...
<div class="row">
    <div class="col-md-12 quote-deco-table">
         <table class="table-data-sheet custom_deco_table">
            <tr>
                <td></td> <td><span id="file-upload-heading">{l s='Archivo' mod='customquote'}</span></td>
    ...
    <form method="post" style="width:100%;" enctype="multipart/form-data" 
                    ...
                    <a count="{$count}" class="add_description">{l s='Add Comment'}</a>
                    <br /><textarea style="display:none" id="file_description_{$count}" name="description" ></textarea>
                   </form>
                   <br />
                </td>
    ...
    <div id="layered_ajax_loader" style="display: none;">
		<p id='ajax-loader'><img src="{$img_ps_dir}loader.gif" alt="" /><br />{l s='Loading...' mod='customquote'}</p>
         ...
         <section id="product-decoration" class="page-product-box">
                            ...
                            {if isset($textFields.$key)} 
                                ...
                                <tr>
                                        ...
                                        <td>{foreach $files as $file}
                                            ...
                                            {/foreach}</td>
                                        <td colspan="3">
                                            <span id='deco_info_{$deco_c}'>{$col1_data[0]}</span>
                                        </td>
                                        <td></td>
                                        <td class="cart_total">
                                            <span class="price">$<span id='deco_price_{$deco_c}'>{$col2_info[$inc]}</span></span>
                                                {$inc = $inc+1}
                                        </td> 
                            <td> <form id='delete_file_{$deco_c}' method="post" class="deco_delete" style="width:100%;" >
                                <input name="idproductfile" value="{$productfile_id}" type="hidden"/>
                              <button class="btn-xs btn btn-danger deco_delete" deco_key="{$col1_data[1]}"
                                      deco_id="{$i}" count='{$deco_c}' id="delete_{$deco_c}">Delete</button>
                                </form></td>
            ...
            <input type="hidden" id="decoration_added" value="{$deco_c}" />
