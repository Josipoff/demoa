			...
			switch ($filter['type'])
			{
				...
				case 'quantity':
					$avaibility_list = array(
						...
						$this->translateWord('Sum of All Available Colors', (int)$filter['id_lang']),
						$this->translateWord('Available Later', (int)$filter['id_lang'])
					);
	...
	private function getSelectedFilters()
	{
		...
		if (strpos($_SERVER['SCRIPT_FILENAME'], 'blocklayered-ajax.php') === false || Tools::getValue('selected_filters') !== false)
		{
			...
			if(Tools::getValue('id_manufacturer')){
                        $selected_filters = array('category' => array(),'manufacturer'=> array());
                        $brand = (int)Tools::getValue('id_manufacturer');
                        array_push($selected_filters['manufacturer'],$brand);
                        }  else {                            
		...
		foreach ($_GET as $key => $value)
			if (substr($key, 0, 8) == 'layered_')
			{
				...
				if (isset($res[1]))
				{
					...
					else if ($res[1] == 'quantity')
						$selected_filters['quantity'][] = $key.'_'.$value;
	...
	public function getProductByFilters($selected_filters = array())
	{
		...
		foreach ($selected_filters as $key => $filter_values)
		{
			...
			if (!empty($word) && strlen($word) >= (int)Configuration::get('PS_SEARCH_MINWORDLEN'))
			{
				...
				$query_filters_where .= ' AND sw.word LIKE
					'.($word[0] == '-'
						? ' \''.pSQL(Tools::substr($word, 1, Configuration::get('PS_SEARCH_MAX_WORD_LENGTH'))).'%\''
						: '\''.pSQL(Tools::substr($word, 0, Configuration::get('PS_SEARCH_MAX_WORD_LENGTH'))).'%\''
					);

				if ($word[0] != '-')
					$query_filters_where .= ' AND sw.word LIKE \''.pSQL(Tools::substr($word, 0, Configuration::get('PS_SEARCH_MAX_WORD_LENGTH'))).'%\'';
			}
                        $query_filters_from .=' LEFT JOIN '._DB_PREFIX_.'search_index si ON '
                                . '(si.id_product = p.id_product)'.
                                ' LEFT JOIN '._DB_PREFIX_.'search_word sw ON (sw.id_word = si.id_word  AND sw.id_shop = '.(int)Context::getContext()->shop->id.')';
		...
		if (isset($price_filter) && $price_filter)
		{
			...
			(
				...
				AND p.`price` BETWEEN '.(int)$price_filter['min'].' AND '.(int)$price_filter['max'].'
	...
	public function getFilterBlock($selected_filters = array())
	{
		...
		foreach ($filters as $filter)
		{
			...
			$products = false;
                ...
                if(Tools::getValue('new_products') || Context::getContext()->controller->php_self == 'new-products'){
                   // $query_filters_from = Shop::addSqlAssociation('product', 'p');
                   $sql_query['where'] .= ' AND product_shop.`date_add` > "'.date('Y-m-d', strtotime('-'.(Configuration::get('PS_NB_DAYS_NEW_PRODUCT') ? (int)Configuration::get('PS_NB_DAYS_NEW_PRODUCT') : 20).' DAY')).'" ';					
                }
                if(Tools::getValue('price_drop') || Context::getContext()->controller->php_self == 'prices-drop'){
                  $sql_query['join'] .= 'LEFT JOIN `'._DB_PREFIX_.'specific_price` sp ON (sp.id_product = p.id_product AND sp.id_shop = '.(int)Context::getContext()->shop->id.')';
                   $sql_query['where'] .= ' AND sp.`reduction` > 0';					
                }
                
                if(Tools::getValue('search_str') || Context::getContext()->controller->php_self == 'search'){  
                    if(Tools::getValue('search_query', Tools::getValue('ref')))
                        $words = explode(' ',Tools::getValue('search_query', Tools::getValue('ref')));
                    else
                        $words = explode(' ',Tools::getValue('search_str'));
                    foreach ($words as $key => $word)
			if (!empty($word) && strlen($word) >= (int)Configuration::get('PS_SEARCH_MINWORDLEN'))
			{
				$word = str_replace('%', '\\%', $word);
				$word = str_replace('_', '\\_', $word);
				$sql_query['where'] .= ' AND sw.word LIKE
					'.($word[0] == '-'
						? ' \''.pSQL(Tools::substr($word, 1, Configuration::get('PS_SEARCH_MAX_WORD_LENGTH'))).'%\''
						: '\''.pSQL(Tools::substr($word, 0, Configuration::get('PS_SEARCH_MAX_WORD_LENGTH'))).'%\''
					);
				...
				if ($word[0] != '-')
					$sql_query['where'] .= ' AND sw.word LIKE \''.pSQL(Tools::substr($word, 0, Configuration::get('PS_SEARCH_MAX_WORD_LENGTH'))).'%\'';
			}
                        $sql_query['join'] .=' LEFT JOIN '._DB_PREFIX_.'search_index si ON '
                                . '(si.id_product = p.id_product)'.
                                ' LEFT JOIN '._DB_PREFIX_.'search_word sw ON (sw.id_word = si.id_word  AND sw.id_shop = '.(int)Context::getContext()->shop->id.')';
			...
			switch ($filter['type'])
			{
				...
				case 'quantity':
					$quantity_array = array (
						...
						1 => array('name' => $this->l('Sum of All Selected Colors'), 'nbr' => 0),
						2 => array('name' => $this->l('Quantity'), 'nbr' => 1),
						3 => array('name' => $this->l('Limit Arrival Date'), 'nbr' => 1),
						4 => array('name' => $this->l('Arrival Date'), 'nbr' => 1)
					);
					foreach ($quantity_array as $key => $quantity)
						if (isset($selected_filters['quantity']) && in_array($key, $selected_filters['quantity']))
                                                        if($key==2 || $key == 4)
							$quantity_array[$key]['value'] = $selected_filters['quantity'][$key];
					if (isset($products) && $products)
						foreach ($products as $product)
						{
					...
					if (isset($products) && $products)
					{
						...
						if ($categories_with_products_count || !Configuration::get('PS_LAYERED_HIDE_0_VALUES'))
							$filter_blocks[] = array (
								'type_lite' => 'category',
								'type' => 'category',
								'id_key' => 0, 'name' => $this->l('Sub Categories'),
								'values' => $tmp_array,
								'filter_show_limit' => $filter['filter_show_limit'],
								'filter_type' => $filter['filter_type']
							);
	...
	public function ajaxCall()
	{
		...
		if ($nb_products == 0)
			...
			$product_list = $smarty->fetch(_PS_THEME_DIR_.'product-list.tpl');
                //var_dump($filter_block['filters'][0]['values']);
		$vars = array(
