...
class StockAvailableCore extends ObjectModel
{
	...
	public $quantity = 0;
        public $intransit_available_date;
        public $color_id;
        public $intransit=0;
	...
	public static $definition = array(
		...
		'fields' => array(
			...
			'color_id' => 				array('type' => self::TYPE_INT, 'validate' => 'isInt'),
			'intransit' => 				array('type' => self::TYPE_INT, 'validate' => 'isInt'),
			'intransit_available_date' => 				array('type' => self::TYPE_DATE),
	...
	public static function copyStockAvailableFromShopToShop($src_shop_id, $dst_shop_id)
	{
        ...
        public static function setStockInventory($product_id,$id_shop,$id_shop_group,$quantity,$color_id,
                $intransit,$available_transit_date)
	{
		
		$query = '
			INSERT INTO '._DB_PREFIX_.'stock_available_inventory
			(
				id_product,
				id_shop,
				id_shop_group,
				quantity,
                                color_id,
                                intransit,
                                intransit_available_date
			) VALUES ("'. $product_id .'","'.$id_shop .'","'.$id_shop_group .'","'.$quantity .'","'.$color_id 
                        .'","'.$intransit .'","'.$available_transit_date .'")';

		return Db::getInstance()->execute($query);
	}
        
         public static function deleteStockInventoryBYProductID($product_id)
	{
		
		$query = 'DELETE FROM '._DB_PREFIX_.'stock_available_inventory WHERE
