...
class CartCore extends ObjectModel
{
	...
	public static $definition = array(
		...
		'fields' => array(
			...
			'deco_cost' => 			array('type' => self::TYPE_STRING),
			...
			'date_in_hand' => 				array('type' => self::TYPE_DATE),
	...
	*/
	...
	public function _addCustomization($id_product, $id_product_attribute, $index, $type, $field, $quantity)
	{
        ...
        public function _addCustomizationAdminOrder($id_product, $id_product_attribute, $index, $type, $field, $quantity,$cart_id,$id_address_delivery)
	{
		$exising_customization = Db::getInstance()->executeS('
			SELECT cu.`id_customization`, cd.`index`, cd.`value`, cd.`type` FROM `'._DB_PREFIX_.'customization` cu
			LEFT JOIN `'._DB_PREFIX_.'customized_data` cd
			ON cu.`id_customization` = cd.`id_customization`
			WHERE cu.id_cart = '.(int)$cart_id.'
			AND cu.id_product = '.(int)$id_product.'
			AND in_cart = 0'
		);
		...
		{
			Db::getInstance()->execute(
				'INSERT INTO `'._DB_PREFIX_.'customization` (`id_cart`, `id_product`, `id_product_attribute`, `quantity`,`id_address_delivery`)
				VALUES ('.(int)$cart_id.', '.(int)$id_product.', '.(int)$id_product_attribute.', '.(int)$quantity.', '.(int)$id_address_delivery.')'
			);
                        $id_customization = Db::getInstance()->Insert_ID();
		}
        ...
        public function _updateCustomizationAdminOrder($id_product, $id_product_attribute, $type, $quantity,$cart_id,$id_address_delivery)
	{
		$exising_customization = Db::getInstance()->executeS('
			SELECT cu.`id_customization`, cd.`index`, cd.`value`, cd.`type` FROM `'._DB_PREFIX_.'customization` cu
			LEFT JOIN `'._DB_PREFIX_.'customized_data` cd
		...
		if ($exising_customization)
		{
			...
			foreach ($exising_customization as $customization)
			{
                            Db::getInstance()->execute('UPDATE `'._DB_PREFIX_.'customization` SET in_cart =1 '
                                        . 'WHERE id_customization='.$customization['id_customization'] .' AND id_cart = '.(int)$cart_id.' AND id_product = '.(int)$id_product.'');
                                break;
			}
			$id_customization = $exising_customization[0]['id_customization'];
		}
	...
	public function deleteProduct($id_product, $id_product_attribute = null, $id_customization = null, $id_address_delivery = 0)
	{
		...
		if (isset(self::$_totalWeight[$this->id]))
			unset(self::$_totalWeight[$this->id]);
                ...
                $customization = $this->getProductCustomizationByAttribute((int)$id_product,(int)$id_product_attribute);  
                //var_dump($customization);
                
                foreach ($customization as $custom_id){
                    $id_customization = $custom_id['id_customization'];
	...
	*/
	public function getOrderTotal($with_taxes = true, $type = Cart::BOTH, $products = null, $id_carrier = null, $use_cache = true)
	{
		...
		foreach ($products as $product) // products refer to the cart details
		{
                    //var_dump($product['id_product']);
                        $product_deco_price=array();
                        if(!in_array($product['id_product'], $product_array)){
                            $product_deco_price = $this->getProductCustomization($product['id_product']);  
                            array_push($product_array, $product['id_product']);
                        }
                        $i=0;
                        $custom_id = 0;
                        $j=2;
                        foreach ($product_deco_price as $deco_price){
                            $i++;                            
                            if($custom_id !=$deco_price['id_customization'])
                            {
                                $custom_id = $deco_price['id_customization'];
                                $j = $i+1;
                            }
                            if($i>$j && $i%2==0){
                            //var_dump($i,$deco_price);
                                $price_array = explode(";", $deco_price['value']);
                                foreach ($price_array as $pr)
                                    $order_total +=floatval($pr);     
                            }
	...
	public function getAdminOrderTotal($id_cart,$with_taxes = true, $type = Cart::BOTH, $products = null, $id_carrier = null, $use_cache = true)
	{
            	if (!$this->id)
			return 0;
		...
		foreach ($products as $product) // products refer to the cart details
		{
                    //var_dump($product['id_product']);
                        $product_deco_price=array();
                        if(!in_array($product['id_product'], $product_array)){
                            $product_deco_price = $this->getAdminProductCustomization($product['id_product'],$id_cart);  
                            array_push($product_array, $product['id_product']);
                        }
                        $i=0;
                        $custom_id = 0;
	...
	*/
	...
	public function getProductCustomization($id_product, $type = null, $not_in_cart = false)
	{
        ...
        public function getAdminProductCustomization($id_product,$id_cart, $type = null, $not_in_cart = false)
	{
		if (!Customization::isFeatureActive())
			return array();
        ...
        public function getProductCustomizationByAttribute($id_product,$id_product_attribute, $type = null, $not_in_cart = false)
	{
		if (!Customization::isFeatureActive())
			return array();
