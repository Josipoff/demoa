...
class AdminOrdersControllerCore extends AdminController
{
	...
	public function postProcess()
	{
		...
		if (Tools::isSubmit('id_order') && Tools::getValue('id_order') > 0)
		{
			...
			ShopUrl::cacheMainDomainForShop((int)$order->id_shop);
                ...
                /* Update In-hand date */
                if (Tools::isSubmit('submitInHandDate') && isset($order))
		{
			if ($this->tabAccess['edit'] === '1')
			{
				$in_hand_date =date("Y-m-d H:i:s", strtotime(Tools::getValue('date_in_hand')));
                                OrderCore::updateInhandDate(Tools::getValue('id_order'),$in_hand_date);
	...
	public function renderView()
	{
		...
		$this->tpl_view_vars = array(
			'order' => $order,
                        'files' =>$orderfiles->adminorderpage_orderfiles($order->id),
	...
	public function ajaxProcessAddProductOnOrder()
	{
		...
		if ($order->hasInvoice())
		{
			...
			if ($order_invoice->id == 0)
			{
				...
				$order_invoice->total_paid_tax_excl = Tools::ps_round((float)$cart->getAdminOrderTotal($order->id_cart, false, $total_method), 2);
				$order_invoice->total_paid_tax_incl = Tools::ps_round((float)$cart->getAdminOrderTotal($order->id_cart, $use_taxes, $total_method), 2);
				$order_invoice->total_products = (float)$cart->getAdminOrderTotal($order->id_cart, false, Cart::ONLY_PRODUCTS);
				$order_invoice->total_products_wt = (float)$cart->getAdminOrderTotal($order->id_cart, $use_taxes, Cart::ONLY_PRODUCTS);
		...
		$order->total_products += (float)$cart->getAdminOrderTotal($order->id_cart, false, Cart::ONLY_PRODUCTS);
		$order->total_products_wt += (float)$cart->getAdminOrderTotal($order->id_cart, $use_taxes, Cart::ONLY_PRODUCTS);

		$order->total_paid += Tools::ps_round((float)($cart->getAdminOrderTotal($order->id_cart, true, $total_method)), 2);
		$order->total_paid_tax_excl += Tools::ps_round((float)($cart->getAdminOrderTotal($order->id_cart, false, $total_method)), 2);
		$order->total_paid_tax_incl += Tools::ps_round((float)($cart->getAdminOrderTotal($order->id_cart, $use_taxes, $total_method)), 2);
		...
		// discount
		$order->total_discounts += (float)abs($cart->getAdminOrderTotal($order->id_cart, true, Cart::ONLY_DISCOUNTS));
		$order->total_discounts_tax_excl += (float)abs($cart->getAdminOrderTotal($order->id_cart, false, Cart::ONLY_DISCOUNTS));
		$order->total_discounts_tax_incl += (float)abs($cart->getAdminOrderTotal($order->id_cart, true, Cart::ONLY_DISCOUNTS));
