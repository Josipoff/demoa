...
class ProductControllerCore extends FrontController
{
	...
	public function initContent()
	{
		...
		if (!$this->errors)
		{
                    ...
                    $parameter =array("ItemNumber"=>$this->product->item_number,
                                    "key"=>EXIMAGEN_KEY);
			...
			$this->context->smarty->assign('packs', Pack::getPacksTable($this->product->id, $this->context->language->id, true, 1));
                        ...
                        $product_info_pdf = array("name"=> $this->product->name,
                        ...
                        $inventory_column = array( array("title"=> "Producto", "key"=> "SKU"),
                            array("title"=> "Fiscal", "key"=> "WHS01"),
                            array("title"=> "Cedis", "key"=> "WHS02"),
                            array("title"=> "DF", "key"=> "WHS30"),
                            array("title"=> "Suma", "key"=> "WHSEXT"),
                            array("title"=> "Apartado", "key"=> "Sum"),
                            array("title"=> "Disponible", "key"=> "Commited"),
                            array("title"=> "Compras", "key"=> "Available"),
                            array("title"=> "Fecha de Llegada", "key"=> "OnTransit"));
                        ...
                        $quick_quote_column = array( array("title"=> "Piezas Pedido", "key"=> "piezas"),
                            array("title"=> "Precio Producto", "key"=> "precio"),
                            array("title"=> "Precio Producto + Impresion", "key"=> "precioImp"));
			...
			if (isset($this->category->id) && $this->category->id)
				...
				'product' => $this->product,
                                ...
                                'quick_quote_column' => $quick_quote_column,
                                'quick_quote_data' => $quick_quote_data,
                                'product_info_pdf' => json_encode($product_info_pdf),
				...
				'show_request_sample' =>  (int)Configuration::get('PRODUCT_REQUEST_SAMPLE'),
				...
				'HOOK_PRODUCT_FILE_CONTENT' => Hook::exec('productFileContent', array('product' => $this->product)),
				'HOOK_PRODUCT_FILE' =>  Hook::exec('productFile', array('product' => $this->product)),
				'HOOK_CUSTOM_QUOTE' =>  Hook::exec('displayProductcustomquote', array('product' => $this->product,'customizationFields' => $customizationFields)),
				'HOOK_PRODUCT_DOWNLOAD' => $product_download_btn,
				...
				'errors' => $this->errors,
                                'quote_table_detail' => $quote_table,
                                'inventory' => $inventory,
	...
	protected function assignImages()
	{
		...
		$image_link_array = array();
                    foreach ($product_images as $image){
                       $imageIds = $image['id_image'];
                       $image_url = $this->context->link->getImageLink($this->product->link_rewrite, $imageIds, 'cart_default');
                       array_push($image_link_array,$image_url);
                    }
                if (count($product_images))
			$this->context->smarty->assign('images', $product_images);
                $this->context->smarty->assign('images_pdf',json_encode($image_link_array));                                
	}
	...
	protected function assignAttributesGroups()
	{
		...
		if (is_array($attributes_groups) && $attributes_groups)
		{
			...
			foreach($colors as $color){
                            array_push($color_array,$color['name']);
                        }
                        $product_color = implode(",", $color_array);
                        $this->context->smarty->assign(array(
				'groups' => $groups,
