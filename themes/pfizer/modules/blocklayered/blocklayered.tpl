		...
		<form action="#" id="layered_form">
			<div>
				{if isset($selected_filters) && $n_filters > 0}
                                    <input type='button' value="{l s='Delete All Filters' mod='blocklayered'}" 
                                           class="btn btn-xs btn-danger" id='delete-all'  />
									...
									{if $filter.type == $filter_type && isset($filter.values)}
										{if isset($filter.slider)}
											{if $smarty.foreach.f_values.first}
												<li>
													<a href="#" rel="layered_{$filter.type}_slider" title="{l s='Cancel' mod='blocklayered'}"><i class="icon-remove"></i></a>
													{if $filter.format == 1}
										...
										{else}
                                                                                    {if $filter.type == 'quantity'}
                                                                                        {assign var=quantity_info value="_"|explode:$filter_value}
                                                                                        {foreach from=$filter.values key=id_value item=value}
                                                                                            	{if $id_value == $quantity_info[2] && $quantity_info[3]!=""}
                                                                                                    {if $id_value==2 || $id_value==4}
													<li>
														<a href="#" rel="layered_{$filter.type_lite}_{$id_value}" title="{l s='Cancel' mod='blocklayered'}"><i class="icon-remove"></i></a>
														{l s='%1$s: %2$s' mod='blocklayered' sprintf=[$value.name, $quantity_info[3]]}
                                                                                                                {if $id_value==2}
                                                                                                                    {assign var=quan_2 value=$quantity_info[3]}
                                                                                                                {else}
                                                                                                                    {assign var=quan_4 value=$quantity_info[3]}
                                                                                                                {/if}
													</li>
                                                                                                    {else}
                                                                                                        <li>
                                                                                                                ...
                                                                                                                {if $id_value==1}
                                                                                                                    {assign var=quan_1 value=$quantity_info[3]}
                                                                                                                {else}
                                                                                                                    {assign var=quan_3 value=$quantity_info[3]}
                                                                                                                {/if}
					...
					{if isset($filter.values)}
						...
						<ul id="ul_layered_{$filter.type}_{$filter.id_key}" class="col-lg-12 layered_filter_ul{if isset($filter.is_color_group) && $filter.is_color_group} color-group{/if}">
								...
								{if $filter.filter_type == 0}
									{foreach from=$filter.values key=id_value item=value name=fe}
										...
										<li class="nomargin {if $smarty.foreach.fe.index >= $filter.filter_show_limit}hiddable{/if} 
                                                                                    {if ($filter.type != 'quantity')}col-lg-6{else}col-lg-12{/if}">
											...
											{else}
                                                                                          {if ($filter.type == 'quantity' && ($id_value<=1 || $id_value==3)) || $filter.type != 'quantity'}
												<input type="checkbox" class="checkbox" name="layered_{$filter.type_lite}_{$id_value}" 
                                                                                                       id="layered_{$filter.type_lite}{if $id_value || $filter.type == 'quantity'}_{$id_value}{/if}" 
                                                                                                       value="{if ($filter.type == 'quantity' && $id_value == 3 )}1{else}{$id_value}{/if}
                                                                                                       {if $filter.id_key}_{$filter.id_key}{/if}"{if isset($value.checked)} checked="checked"{/if}
                                                                                                       {if $id_value==1 && isset($quan_1)} checked="checked" {/if}{if $id_value==3 && isset($quan_3)} checked="checked" {/if}
											...
											<label for="layered_{$filter.type_lite}_{$id_value}"{if !$value.nbr} class="disabled"{else}{if isset($filter.is_color_group) && $filter.is_color_group} name="layered_{$filter.type_lite}_{$id_value}" class="layered_color" rel="{$id_value}_{$filter.id_key}"{/if}{/if}>
												...
												{else}
												<a href="{$value.link}"{if $value.rel|trim != ''} rel="{$value.rel}"{/if}>{$value.name|escape:'html':'UTF-8'}{if $layered_show_qties}<span>{if (($filter.type == 'quantity' && $id_value != 3 )||$filter.type == 'category')} ({$value.nbr}) {/if}</span>{/if}</a>
												{/if}
											</label>
                                                                                        {/if}
                                                                                          {if $filter.type == 'quantity' && $id_value>1}
                                                                                              {if $id_value==2}
                                                                                              <input id="layered_{$filter.type}_range_min" type="text" value="{if isset($quan_2)}{$quan_2}{/if}"
                                                                                                     class="form-control layered_input_range layered_input_range_min" placeholder="{$value.name}" 
                                                                                                     name="layered_{$filter.type_lite}_{$id_value}" />
                                                                                              {/if}
                                                                                              {if $id_value==4}
                                                                                               <input id="layered_{$filter.type}_available_date" type="text" value="{if isset($quan_4)}{$quan_4}{/if}" disabled
                                                                                                     class="form-control datepicker layered_input_range layered_input_range_max" placeholder="{$value.name}" 
                                                                                                     name="layered_{$filter.type_lite}_{$id_value}" />
                                                                                               {/if}
                                                                                          {/if}
										</li>
							...
							{else}
								{if $filter.filter_type == 0}
                                                                    <span id="errmsg"></span>
                                                                            <label>{l s='Price Min:' mod='blocklayered'}</label>
                                                                            <input class='form-control price_input' type="text" id='price_min' />
                                                                            <label>{l s='Price Max:' mod='blocklayered'}</label>
                                                                            <input class='form-control price_input' type="text" id='price_max' />
