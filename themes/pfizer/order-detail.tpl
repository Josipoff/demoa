...
<div class="info-order box">
	<p><strong class="dark">{l s='In-Hand Date'}</strong> <span class="color-myaccount">{dateFormat date=$order->date_in_hand full=0}</span></p>
...
{if isset($receipt_confirmation) && $receipt_confirmation}
    <p class="success">
        {l s='Thank you for your confirmation! Your order is ready for production.'}
    </p>
{/if}

{if $order_history.0.id_order_state == 15}
    <form action="{$link->getPageLink('order-detail', true)|escape:'html'}" method="post" class="std" id="markAsReceived">
        ...
        <input type="submit" class="exclusive" name="markAsReceived" id="markAsReceivedBtn" value="{l s='Confirm Order and Decoration'}">
        <span>{l s='Si estas de acuerdo con el decorado propuesto confirma con el boton. Si deseas modificaciones al envía un mensaje especificando el producto en en la parte de abajo'}</span>
        <p class="clear"></p>
    </form>
...
<div class="row">
            <div class="col-lg-12">
                 <div id='deco_upload_form' style='display:none'> 
                      <h3>Upload New Decoration</h3>
      <form action="" method="POST" enctype="multipart/form-data">
              <table><tr><td><input type="file" name="file[]" /></td>
                    <td><input type="hidden" name="filemanager"/>
                        <input type="hidden" name="oid" value="{$order->id_cart}" />
                        <input type="hidden" name="id_order" value="{$order->id}" />
                        <input type="hidden" id='file_id' name="id" value="" />
                        <input type="hidden" id='product_id' name="idproduct" value="" />
                        <input type="hidden" id='deco_key' name="deco_key" value="" />
                        <textarea id="file_description_{$count}" name="description" ></textarea>
                        <input type="submit" name="addfile" class="button" value="{l s='upload'}">
                      </td></tr></table>
      </form><br /></div></div>
        </div>
					...
					{if isset($customizedDatas.$productId.$productAttributeId)}
						{foreach $customizedDatas.$productId.$productAttributeId[$product.id_address_delivery] as $id_customization=>$customization}
								...
								class="product_customization_for_{$product.id_product}_{$product.id_product_attribute}_{$product.id_address_delivery|intval}{if $odd} odd{else} even{/if} customization alternate_item {if $product@last && $customization@last && !count($gift_products)}last_item{/if}">
                                                  ...
                                                  <a class="upload_file_preview" id='upload_file_preview' href='#file_preview'
                                                     filename="{$content_dir}/modules/orderfiles/productfiles/{$file['cookieid']}/{$file['filename']}">
                                                     <img class="file-thumbnail imgm img-thumbnail" alt="" 
                                                          src="{$content_dir}/modules/orderfiles/productfiles/{$file['cookieid']}/{$file['filename']}"/>
                                ...
                                Download
                                          ...
                                          {/foreach}         <br />
                                                                 ...
                                                                 <a class="upload_image_link" href="#deco_upload_form" file_id="{$file_id}" product_id="{$product.product_id}"
                           deco_key="{$col1_data[1]}">
                            Upload New Decoration</a>
                                                           </td>
                                                            <td colspan="2">
                                                                        {$col1_data[0]}
								</td>
				...
				<tr class="{if $smarty.foreach.messageList.first}first_item{elseif $smarty.foreach.messageList.last}last_item{/if} {if $smarty.foreach.messageList.index % 2}alternate_item{else}item{/if}">
					<td>
						...
						{dateFormat date=$message.date_add full=1}
                                        ...
                                        <td><strong>Product: {$message.product_name}</strong><br />{$message.message|escape:'html':'UTF-8'|nl2br}</td>
				</tr>
			{/foreach}
			</tbody>
		</table>
