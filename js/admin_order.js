...
$(document).ready(function() {
	// Init all events
	init();
         $(document).on('change','.calc_input',function(e){
            enable_add_btn();
            reloadQuote(true);
        });
        
        $(document).on('click','.upload_image_link',function(e){
            $('#deco_upload_form').show();
            $("#file_id").val($(this).attr('file_id'));
            $("#product_id").val($(this).attr('product_id'));
            $("#deco_key").val($(this).attr('deco_key'));
        });
        
        $(document).on('blur', '#quan', function(e)
                {
                    $('#add_product_product_quantity').val($('#quan').val());
                   reloadQuote(true);
                });
                
        $(document).on('keyup', '#add_product_product_quantity', function(e)
       {
           $('#quan').val($('#add_product_product_quantity').val());
          reloadQuote(true);
       });

         $(document).on('click','#upload_file_preview',function(){
    var filename = $(this).attr("filename");
    $('#image_upload_preview').prop("src",filename);
});
                
        $(document).on('click','.deco_add',function(e){
           var deco_id=parseInt($(this).attr('deco_id'));
           var deco_id1 = (deco_id)*2;
           var deco = $('#textField'+deco_id1).val();
           if(deco!='')
               deco +=';';
           deco +=$('#area_'+deco_id).html()+','+$('#tec_'+deco_id).html()+',';
           deco += $('#size_'+deco_id).html().substring(0,1)+','+$('#color_'+deco_id).val();
           deco +="-"+Math.floor(Math.random()*100000);
           $('#textField'+deco_id1).val(deco);
           
           var deco_price = $('#textField'+(deco_id1+1)).val();
           if(deco_price!='')
               deco_price +=';';
           deco_price +=$('#tdeco_'+deco_id).html();
           $('#textField'+(deco_id1+1)).val(deco_price);
           var count = $("#textField0").val();
           if(count=='')
               count = 1;
           else
               count = parseInt(count)+1;
           $("#textField0").val(count);
           var cal = 2;
           var deco_c =1;
           var table_data ='<table style="width:100%">';
           for(i=1;i<=parseInt($('#max_deco').val());i++){
               deco_array = $('#textField'+cal).val().split(';');
               cal++;
               deco_price_array = $('#textField'+cal).val().split(';');
               cal++;
               var j=0;
               if(!(deco_array.length==1 && deco_array[0]=='')){
               for(j=0;j<deco_array.length;j++){
                   deco_info =deco_array[j].split('-');
               table_data +='<tr><td id="deco_info_'+deco_c+'">'+deco_info[0]+'</td><td id="deco_price_'+deco_c+'">'+deco_price_array[j]+'</td><td><button class="btn-xs btn btn-danger ';
               table_data +='deco_delete" deco_key="'+deco_info[1]+'" deco_id="'+i+'" count="'+deco_c+'" id="delete_'+deco_c+'">Delete</button></td></tr>';
               deco_c++;
           }}
        }table_data +='</table>';
        $('.deco-block').html(table_data);
        $('#add_product_product_quantity').prop('disabled',true);
        $('#quan').prop('disabled',true);
           return false;
        });
        
        $(document).on('click','.deco_delete',function(e){
           var deco_id=parseInt($(this).attr('deco_id'));
           if(deco_id){
           var deco_id1 = (deco_id*2);
           ...
           if(deco_array[0]==deco_str)
               if(deco_array.length==1)
           ...
           if(deco_price_array[0]==deco_price_str)
               if(deco_price_array.length==1)
           ...
           if(count==0){
           $("#textField0").val("");
            $('#add_product_product_quantity').prop('disabled',false);
...
function reloaddecorationBlock(id_product){
    var ajax_query = $.ajax({
            type: 'POST',
            url: baseDir + 'modules/customquote/customquote-admin-content.php',
            cache: false,
            dataType: 'html',
            data : "product_id="+id_product,
            success : function(data) {
                $("#add_product_product_decoration_id").html(data);
                reloadQuote(true);
            }
    });
}
		...
		success: function(result)
		{
                    ...
                    for(i=1;i<=parseInt($('#max_deco').val());i++){
                        ...
                        if($(check_id).is(':checked')){
                            ...
                            $('#tdeco_'+i).html(parseFloat((tdeco[count])/profitMargin).toFixed(2));
                            deco += $('#tec_'+i).html() + ', ';
                            count++;
                        }else{
                            $('#print_'+i).html("0");
                    ...
                    $('#psingle').html(parseFloat((result['ProductPriceSingle'])/profitMargin).toFixed(2));
                    $('#pprice').html(parseFloat((result['ProductPrice'])/profitMargin).toFixed(2));
                    $('#setup_single').html(parseFloat((result['SetupChargeSingle'])/profitMargin).toFixed(2));
                    $('#psetup').html(parseFloat((result['SetupCharge'])/profitMargin).toFixed(2));
                    $('#pdeco_single').html(parseFloat((result['DecoratedPriceSingle'])/profitMargin).toFixed(2));
                    $('#pdeco').html(parseFloat((result['DecoratedPrice'])/profitMargin).toFixed(2));
                    $('#tsingle').html(parseFloat((result['TotalSingle'])/profitMargin).toFixed(2));
                    $('#total').html(parseFloat((result['Total'])/profitMargin).toFixed(2));
                    $('#mprice').html(parseFloat((result['TotalSingle'])/profitMargin).toFixed(2));
                    $('#mprice_tot').html(parseFloat((result['Total'])/profitMargin).toFixed(2));
                    $('#ajax-loader').remove();
...
function enable_add_btn(){
    ...
    for(i=1;i<=parseInt($('#max_deco').val());i++){
        ...
        $("#textField1").val(checkboxID.substring(0,checkboxID.length-1));
}
...
function closeAddProduct()
{
	...
	$('#add_product_product_decoration_area').hide();
	if (stock_management)
...
function init()
{
	...
	.result(function(event, data, formatted) {
		...
		else
		{
			...
			addProductRefreshTotal();
                        reloaddecorationBlock(data.id_product);                        
			if (stock_management)
			...
			if (current_product.combinations.length !== 0)
			{
				...
				$('#add_product_product_attribute_area').show();
                                $('#add_product_product_decoration_area').show();
		...
		if (go)
		{
			...
			if (go)
			{
				var query = 'ajax=1&token='+token+'&action=addProductOnOrder&id_order='+id_order+'&';
                                j=0;
                                query +='textField'+j+'='+$('#textField'+j).val()+'&';
                                j=j+1;                                     
                                query +='textField'+j+'='+$('#textField'+j).val()+'&';
                                j++;
                                 for(i=1;i<=parseInt($('#max_deco').val());i++){
                                ...
                                if($('#add_product_product_quantity').prop('disabled')){
                                    query +='add_product[product_quantity]='+$('#add_product_product_quantity').val()+'&';
                                }
				...
				var ajax_query = $.ajax({
					...
					error : function(XMLHttpRequest, textStatus, errorThrown) {
						jAlert("Impossible to add the product to the cart.\n\ntextStatus: '" + textStatus + "'\nerrorThrown: '" + errorThrown + "'\nresponseText:\n" + XMLHttpRequest.responseText);
                                    ...
                                    window.location.href=admin_order_tab_link+admin_order_url;
                                });
	...
	$('.edit_shipping_number_link').unbind('click').click(function(e) {
        ...
        $("#date_in_hand").datepicker({
        prevText: "",
        nextText: "",
        dateFormat: "dd-mm-yy"});

        
        $('.edit_in_hand_date').unbind('click').click(function(e) {
		$(this).parent().find('.in_hand_show').hide();
		$(this).parent().find('.in_hand_edit').show();

		$(this).parent().find('.edit_in_hand_date').hide();
		$(this).parent().find('.cancel_in_hand_date').show();
		e.preventDefault();
	});
        $('.cancel_in_hand_date').unbind('click').click(function(e) {
		$(this).parent().find('.in_hand_show').show();
		$(this).parent().find('.in_hand_edit').hide();

		$(this).parent().find('.edit_in_hand_date').show();
		$(this).parent().find('.cancel_in_hand_date').hide();
		e.preventDefault();
	});
